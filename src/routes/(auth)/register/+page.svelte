<script lang="ts">
	...
	let phone = $state('');
	let password = $state('');
	let birthday = $state(new CalendarDate(2000, 1, 1));
	...
	<AlertDialog.Portal>
		...
		<AlertDialog.Content class="fixed left-1/2 top-1/2 z-50 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-[15px] border border-zinc-800 bg-zinc-900 p-6">
			...
			<div class="mt-6 flex gap-3">
				...
				<AlertDialog.Action onclick={register} class="flex-1 rounded-[15px] bg-blue-600 py-2.5 text-sm font-medium text-white hover:bg-blue-500 transition-colors">I agree</AlertDialog.Action>
			</div>
		</AlertDialog.Content>
	</AlertDialog.Portal>
</AlertDialog.Root>
...
<div class="flex min-h-screen bg-zinc-950">
	<div class="flex flex-1 items-center justify-center px-6 py-12">
		<div class="w-full max-w-sm">
			...
			{#if success}
			<Alert.Root class="mb-4 rounded-[15px] border-green-800 bg-green-950/50">
			...
			<form class="flex flex-col gap-3" onsubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
				<div class="relative">
					...
					<Input type="text" placeholder="Full name *" bind:value={name} class="rounded-[15px] bg-zinc-900 border-zinc-800 pl-9 h-12 text-white placeholder:text-zinc-500" />
				</div>

				<div class="relative">
					<span class="absolute left-3.5 top-1/2 -translate-y-1/2 text-sm text-zinc-500">@</span>
					<Input type="text" placeholder="Username *" bind:value={username} class="rounded-[15px] bg-zinc-900 border-zinc-800 pl-8 h-12 text-white placeholder:text-zinc-500" />
				</div>

				<div class="relative">
					<Mail size={15} class="absolute left-3.5 top-1/2 -translate-y-1/2 text-zinc-500" />
					<Input type="email" placeholder="Email (optional)" bind:value={email} class="rounded-[15px] bg-zinc-900 border-zinc-800 pl-9 h-12 text-white placeholder:text-zinc-500" />
				</div>

				<div class="relative">
					<Phone size={15} class="absolute left-3.5 top-1/2 -translate-y-1/2 text-zinc-500" />
					<Input type="tel" placeholder="Phone number (optional)" bind:value={phone} class="rounded-[15px] bg-zinc-900 border-zinc-800 pl-9 h-12 text-white placeholder:text-zinc-500" />
				</div>
				...
				<DateField.Root bind:value={birthday}>
					<div class="flex flex-col gap-1.5">
						...
						<DateField.Input class="flex h-12 w-full items-center rounded-[15px] border border-zinc-800 bg-zinc-900 px-3 text-sm text-white">
							{#snippet children({ segments })}
								{#each segments as { part, value }, i (part + i)}
									<div class="inline-block">
										...
										{:else}
											<DateField.Segment {part} class="rounded-[8px] px-1 py-0.5 hover:bg-zinc-800 focus:bg-zinc-800 focus:text-white aria-[valuetext=Empty]:text-zinc-600 focus-visible:ring-0">{value}</DateField.Segment>
										{/if}
									</div>
								{/each}
							{/snippet}
